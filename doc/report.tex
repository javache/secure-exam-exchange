...
see the same exam that the professor posted, without any alterations, and be
able to verify that it was authored by the professor. Furthermore, only the
selected students should be able to see the exam, and we have to make sure they
cannot view the exam before they are allowed to.
...
will be detected) and that they cannot be repudiated. These answers are also
confidential and should only be readable to the professor.
...
which notes the student's identity, the server time, the exam and a SHA5-12 hash
...
After submission the student will receive a proof of submission, to complete the
audit trail. Again the proof is a text file, signed with the server's private
key. This time the text file contains the student's identify, the server time, a
reference the exam and a SHA-512 hash of the encrypted answers file that was
just submitted.
