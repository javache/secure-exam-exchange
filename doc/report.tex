...
But even if our platform does not contain a single vulnerability, students can
still attempt to cheat in other ways. They could set up a communication channel
outside of our platform (mobile phones, passing notes) and this cannot be
prevented by our application.

This is why we must take a number of practical decisions.

If the students take the exam in a supervised room, we can prevent them from
using communication using e.g., passing notes.
...
It such a setup, it would be advisable to use public computers instead of having
the students bringing their own laptops: this ensures that the students do not
bring more information than allowed for the exam. They do need to bring their
private key on a USB stick in this case, it would not be a bad idea to encrypt
it with a passphrase in case it gets stolen.
...
Several attacks are possible at this level:
\begin{itemize}
  \item One could impersonate our server
  \item Man in the middle
  \item Replay attack
  \item Traffic sniffing
\end{itemize}

However, since we are using HTTPS, none of these attacks are possible.
